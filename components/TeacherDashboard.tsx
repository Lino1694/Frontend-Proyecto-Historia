...
const TeacherDashboard: React.FC<TeacherDashboardProps> = ({ onLogout }) => {
    ...
    return (
        <div className="flex h-full bg-brand-cream">
            {/* Sidebar for desktop */}
            <aside className="hidden lg:flex lg:flex-col lg:w-64 lg:bg-brand-cream lg:shadow-lg">
                ...
                <div className="p-4 border-t border-slate-200">
                    ...
                    <button onClick={onLogout} className="w-full bg-brand-dark-green/20 text-brand-dark-green font-semibold px-3 py-2 rounded-lg hover:bg-brand-dark-green/30 transition-colors">
                        Salir
                    </button>
                </div>
            </aside>
                ...
                <div className="fixed inset-0 bg-black/50 z-50 lg:hidden" onClick={() => setMobileMenuOpen(false)}>
                    <div className="absolute right-4 top-16 bg-white rounded-lg shadow-lg p-4 min-w-48" onClick={(e) => e.stopPropagation()}>
                        <nav className="space-y-2">
                            <button onClick={() => { navigateTo('home'); setMobileMenuOpen(false); }} className="flex items-center gap-3 w-full p-2 rounded-lg text-slate-700 hover:bg-brand-green/10">
                                <HomeIcon className="h-5 w-5" />
                                <span>Inicio</span>
                            </button>
                            <button onClick={() => { navigateTo('content_management'); setMobileMenuOpen(false); }} className="flex items-center gap-3 w-full p-2 rounded-lg text-slate-700 hover:bg-brand-green/10">
                                <DocumentTextIcon className="h-5 w-5" />
                                <span>Contenido</span>
                            </button>
                            <button onClick={() => { navigateTo('reports'); setMobileMenuOpen(false); }} className="flex items-center gap-3 w-full p-2 rounded-lg text-slate-700 hover:bg-brand-green/10">
                                <ChartBarIcon className="h-5 w-5" />
                                <span>Reportes</span>
                            </button>
                            <button onClick={() => { navigateTo('class_management'); setMobileMenuOpen(false); }} className="flex items-center gap-3 w-full p-2 rounded-lg text-slate-700 hover:bg-brand-green/10">
                                <UsersIcon className="h-5 w-5" />
                                <span>Clase</span>
                            </button>
                            <button onClick={() => { navigateTo('challenges'); setMobileMenuOpen(false); }} className="flex items-center gap-3 w-full p-2 rounded-lg text-slate-700 hover:bg-brand-green/10">
                                <TrophyIcon className="h-5 w-5" />
                                <span>Retos</span>
                            </button>
                            <button onClick={() => { navigateTo('dynamic_generation'); setMobileMenuOpen(false); }} className="flex items-center gap-3 w-full p-2 rounded-lg text-slate-700 hover:bg-brand-green/10">
                                <SparklesIcon className="h-5 w-5" />
                                <span>Generación IA</span>
                            </button>
                            <button onClick={() => { navigateTo('rewards_center'); setMobileMenuOpen(false); }} className="flex items-center gap-3 w-full p-2 rounded-lg text-slate-700 hover:bg-brand-green/10">
                                <SparklesIcon className="h-5 w-5" />
                                <span>Insignias</span>
                            </button>
                            <button onClick={() => { navigateTo('profile'); setMobileMenuOpen(false); }} className="flex items-center gap-3 w-full p-2 rounded-lg text-slate-700 hover:bg-brand-green/10">
                        ...
                        <div className="border-t border-slate-200 mt-4 pt-4">
                            <p className="text-sm text-slate-600 mb-2">Hola, {user?.nombre || 'Usuario'}</p>
                            <button onClick={() => { onLogout(); setMobileMenuOpen(false); }} className="w-full bg-brand-dark-green/20 text-brand-dark-green font-semibold px-3 py-2 rounded-lg hover:bg-brand-dark-green/30 transition-colors">
                                Salir
                            </button>
                        </div>
                    </div>
            ...
            <main className="flex-1 flex flex-col">
                {/* Mobile header */}
                <div className="lg:hidden p-4 bg-brand-cream shadow-sm flex items-center justify-between">
